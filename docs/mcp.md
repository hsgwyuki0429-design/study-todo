...
| `voidStudyRecords` | 記録を取り消す（消さずに「取り消し済み」として残す） |
| `voidChallengeResults` | チャレンジ1回ぶんを履歴から取り消す（結果と中の記録をまとめて） |
| `restoreStudyRecords` | 取り消した記録・チャレンジを、取り消す前の状態へ戻す |
...
- `voidStudyRecords` は消さずに「取り消し済み」にします。集計からは外れ、履歴には残ります。
  古い端末が同期しても、取り消した記録が復活することはありません（版で重ね合わせるため）。
...
| 1回ぶんを履歴から取り消す | `voidChallengeResults`（結果と中の記録をまとめて） |
| 中の1問だけ取り消す | `voidStudyRecords`。チャレンジの回そのものは残り、その1問だけが集計から外れます |
| 中の1問の評価を直す | `updateStudyRecords`（評価だけ） |
| 中の1問の日付・所要時間を直す | できません（`challenge_record` で断ります） |
| 取り消しを戻す | `restoreStudyRecords`（記録・チャレンジのどちらも） |

測った合計時間は、1問を取り消しても変えません（測った事実だからです）。

### 取り消しを戻す

取り消しは削除ではないので、いつでも戻せます。

```json
{ "operationId": "restore-01", "records": ["rec_..."], "challenges": ["chl_..."] }
```

- チャレンジを1回ぶん戻すと、**そのとき道連れで取り消した記録だけ**が戻ります。
  先に1問ずつ取り消してあった分は、取り消したままになります（本人が別に決めたことなので勝手に戻しません）。
- 取り消していないものを戻そうとすると `not_voided` で断ります。
- 何を取り消したかは `getRecordChanges` で分かります。

アプリからも同じことができます。スケジュールで日付を押すと、

- チャレンジの行の「取り消す」… その回ぶんを取り消す
- 中の1問を押して「この1問の記録を取り消す」… その1問だけ取り消す（回は残る）
- いちばん下の「取り消したもの」… 「戻す」で元に戻す
...
| 間違った記録が入った | AIに取り消しを頼む（`voidStudyRecords`）。記録は消えず「取り消し済み」になります |
