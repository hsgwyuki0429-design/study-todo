...
| 1回ぶんを履歴から取り消す | `voidChallengeResults`（結果と中の記録をまとめて取り消す） |
| 中の1問の評価を直す | `updateStudyRecords`（評価だけ） |
| 中の1問の日付・所要時間を直す | できません（`challenge_record` で断ります） |
| 中の1問だけ取り消す | できません。`voidStudyRecords` にその記録を渡すと、**その回ごと**取り消されます |

アプリからも同じことができます。スケジュールで日付を押し、チャレンジの行の「取り消す」を押すと
1回ぶんが履歴から外れます（記録タブ・スケジュールのマスの中も同じです）。
